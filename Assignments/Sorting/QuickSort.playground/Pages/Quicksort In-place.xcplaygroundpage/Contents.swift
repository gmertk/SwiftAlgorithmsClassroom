...
func lomutoPartition<T>(inout array: [T], low: Int, high: Int, @noescape _ isOrderedBefore: (T, T) -> Bool) -> Int {
    ...
    for j in low...high-1 where isOrderedBefore(array[j], pivot) {
        if (i != j) {
...
func hoarePartition<T>(inout array: [T], low: Int, high: Int, @noescape _ isOrderedBefore: (T, T) -> Bool) -> Int {
    ...
    while true {
        repeat {
            left += 1
        } while (left < high && isOrderedBefore(array[left], pivot))

        repeat {
            right -= 1
        } while isOrderedBefore(pivot, array[right])
...
//This doesn't work right but sorting works fine (could see below)
...
func quickSort<T>(inout array: [T], low: Int, high: Int, @noescape _ isOrderedBefore: (T, T) -> Bool) {
    ...
    let partition = hoarePartition(&array, low: low, high: high, isOrderedBefore)
    quickSort(&array, low: low, high: partition - 1, isOrderedBefore)
    quickSort(&array, low: partition + 1, high: high, isOrderedBefore)
}
...
var array7 = [1, 1, 1, 0]
quickSort(&array7, low: 0, high: array7.count - 1, <)
assert(array7 == [0, 1, 1, 1])
/*:
### 1.4. 🇳🇱 problem
...
func dutchFlagPartition<T: Comparable>(inout array: [T], pivotIndex: Int, low: Int, high: Int) -> (Int, Int) {
    ...
    while (equal <= larger) {
        if array[equal] < pivot {
            swap(&array[equal], &array[smaller])
            smaller++
            equal++
        }
